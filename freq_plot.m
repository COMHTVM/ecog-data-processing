...
% x_var = x_clean_line;
%% Select patient
patient = 'TMS No Filter 100% Output';
stim_type = 'artifact_test';
size_x = size(x_var);
...
elseif strcmp(stim_type, 'pre-sham')
    ...
    stim_epochs = 1:50;
elseif strcmp(stim_type, 'artifact_test')
    stim_type_name = ' Artifact Test ';
    stim_epochs = 1:size_x(3);
else 
    stim_type_name = ' Pre-Stimulation Sham ';
    stim_epochs = 101:150;
end
...
for ii = 1:2:size_x(2)
hold on
[pxx, f] = nt_spect_plot(x_var(41000:end,ii,stim_epochs), nsc, nov, nff, Fs);
line = plot(f,abs(pxx).^0.5);
if strcmp(stim_type, 'artifact_test')
line.MarkerSize = ii;
else
...
set(gca,'yscale','log');
xlim([f(1) max_freq]);
xlabel('Hz','FontSize',14); ylabel('Hz^{-0.5}', 'interpreter', 'tex', 'FontSize',14);
...
line = plot(-1000:1/Fs*1000:1000-1/Fs,tep);
if strcmp(stim_type, 'artifact_test')
line.MarkerSize = ii;
else
line.MarkerSize = str2double(EEG.chanlocs(ii).labels); % Using MarkerSize as a dummy numeric variable
...
[x_baseline_detrend]=nt_detrend(x_var,0,[zeros(1,30000) ones(1,8000) zeros(1,42000)]','polynomials',3,3);
